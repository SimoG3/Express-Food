...
import { Plus, Edit2, Trash2, Check, Upload, LogOut, Package, Users, RefreshCw, Key } from 'lucide-react';
    ...
    <div className="max-w-xl mx-auto">
      ...
      <div className="bg-white border rounded-sm" style={{ borderColor: '#e5e5e5', borderTop: `3px solid ${COLOR.red}` }}>
        <div className="p-5 flex flex-col gap-4">
          ...
          {/* Image section */}
          <div>
            <label className="text-xs font-black text-gray-500 uppercase tracking-wide block mb-2">
              Image du produit
            </label>
            <div className="flex gap-2 mb-2">
              {(['url', 'upload'] as const).map(m => (
                <button key={m} onClick={() => setImgMode(m)}
                  ...
                  style={{
                    background:  imgMode === m ? COLOR.red : 'white',
                    color:       imgMode === m ? 'white'   : '#555',
                    borderColor: '#e5e5e5',
                  }}>
                  {m === 'url' ? '🔗 URL' : '📤 Upload'}
                </button>
            ...
            {imgMode === 'url' ? (
              ...
              <div>
                ...
                <button onClick={() => fileRef.current?.click()}
                  ...
                  <Upload size={15} style={{ color: COLOR.red }} /> Choisir une image (PNG, JPG, WEBP)
                </button>
                {form.image?.startsWith('data:') && (
                  <p className="text-xs mt-1" style={{ color: COLOR.green }}>✓ Image Base64 chargée</p>
                )}
              </div>
            )}
          ...
          <div className="grid grid-cols-3 gap-3">
            {([
              ...
              <div key={key} className="flex items-center gap-2 p-2.5 border rounded-sm"
          ...
          <div className="flex items-center gap-3 flex-wrap pt-2 border-t" style={{ borderColor: '#f0f0f0' }}>
            ...
            <button onClick={() => setView('table')}
              className="border px-5 py-2.5 text-sm rounded-sm text-gray-600"
              style={{ borderColor: '#e5e5e5' }}>
              Annuler
            </button>
            {editing && (
              <button onClick={() => { handleDelete(editing.id); setView('table'); }}
                className="ml-auto flex items-center gap-1.5 text-sm font-medium hover:text-red-600"
                style={{ color: '#f87171' }}>
                ...
                <tr key={p.id} className="border-t"
                  ...
                  <td className="px-4 py-3">
                    <SafeImage src={p.image} alt={p.name} fallbackText={p.name}
                      className="w-11 h-11 object-cover rounded-sm border"
                      style={{ borderColor: '#e5e5e5' }} />
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <span className="text-xs font-bold px-2 py-0.5 rounded-sm text-white"
                      style={{ background: COLOR.dark }}>{p.category}</span>
                  </td>
                  <td className="px-4 py-3 font-black whitespace-nowrap" style={{ color: COLOR.red }}>
                    {p.price.toFixed(2)} €
                  </td>
                  <td className="px-4 py-3">
                    {p.discount
                      ? <span className="text-red-500 font-bold text-xs">-{p.discount}%</span>
                      : <span className="text-gray-300 text-xs">—</span>}
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex gap-1">
                      {p.isNew    && <span className="text-xs px-1 py-0.5 rounded-sm text-white font-bold"
                        style={{ background: '#7c3aed' }}>NEW</span>}
                      {p.featured && <span className="text-xs px-1 py-0.5 rounded-sm text-white font-bold"
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1">
                      <button onClick={() => openEdit(p)}
                        ...
                        style={{ color: COLOR.red }} title="Modifier">
                        <Edit2 size={14} />
                      </button>
                      <button onClick={() => handleDelete(p.id)}
                        className="p-1.5 rounded-sm hover:bg-red-50 text-red-400" title="Supprimer">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </td>
...
function ProClientsTab() {
  ...
  const openOverrides = (c: ProClient) => {
    ...
    products.forEach(p => {
      if (c.priceOverrides[p.id] !== undefined) inputs[p.id] = String(c.priceOverrides[p.id]);
    });
      ...
      <div>
        ...
        <div className="flex items-center gap-3 mb-4 flex-wrap">
          <button onClick={() => setView('list')}
            className="text-sm font-bold px-3 py-2 rounded-sm border"
            style={{ borderColor: '#e5e5e5', color: COLOR.red }}>
            ← Retour aux clients
          </button>
          <div>
            <h3 className="font-black text-gray-800">
              Tarifs Pro — {fresh.name}
            </h3>
        ...
        <div className="flex flex-wrap gap-2 mb-4 items-center">
          <input type="text" value={overrideSearch}
            onChange={e => setOverrideSearch(e.target.value)}
            placeholder="Filtrer les produits…"
            className="border px-3 py-1.5 text-sm rounded-sm w-44 focus:outline-none"
            style={{ borderColor: '#e5e5e5' }} />
          <div className="flex gap-1.5 flex-wrap">
            {ALL_CATEGORIES.map(c => (
              <button key={c} onClick={() => setCatFilter(c)}
                ...
                style={{
                  background:  catFilter === c ? COLOR.red : 'white',
                    ...
                    <tr key={p.id} className="border-t"
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <SafeImage src={p.image} alt={p.name} fallbackText={p.name}
                            className="w-9 h-9 object-cover rounded-sm border flex-shrink-0"
                            style={{ borderColor: '#e5e5e5' }} />
                          <span className="font-medium text-gray-800 text-xs max-w-36 truncate">
                            {p.name}
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <input type="number" min="0" step="0.01"
                          ...
                          style={{
                            borderColor: override !== undefined ? COLOR.green : '#e5e5e5',
                            color:       override !== undefined ? COLOR.green  : '#888',
                          }} />
                      </td>
                      <td className="px-4 py-3">
                        {override !== undefined && savings > 0
                          ? <span className="font-bold text-xs" style={{ color: COLOR.green }}>
                              -{savings.toFixed(2)} €
                            </span>
                          : <span className="text-gray-300 text-xs">—</span>}
                      </td>
                      <td className="px-4 py-3">
                        {override !== undefined && (
                          ...
                          }} className="text-xs font-medium hover:text-red-600" style={{ color: '#f87171' }}>
                            Effacer
                          </button>
                        )}
                      </td>
  ...
  return (
    <>
      {/* Add client form */}
      <div className="bg-white border rounded-sm p-5 mb-5"
        style={{ borderColor: '#e5e5e5', borderLeft: `4px solid ${COLOR.red}` }}>
        <h3 className="font-black text-gray-800 mb-4 flex items-center gap-2">
          <RefreshCw size={16} style={{ color: COLOR.red }} />
          Créer un nouveau client Pro
        </h3>
        <div className="flex flex-wrap gap-3">
          ...
          <div className="flex-1 min-w-40">
            ...
            <input type="text" value={newCompany}
        ...
        {/* Last generated */}
        {lastAdded && (
          <div className="mt-3 p-3 rounded-sm border flex items-center gap-3"
            style={{ background: '#fff8f8', borderColor: `${COLOR.red}40` }}>
            <Key size={16} style={{ color: COLOR.red }} />
            <div className="text-sm">
              Client <strong>{lastAdded.name}</strong> créé —&nbsp;
              Code : <strong className="font-black text-lg tracking-widest"
                style={{ color: COLOR.red }}>{lastAdded.accessCode}</strong>
            </div>
            <button onClick={() => setLastAdded(null)}
              className="ml-auto text-gray-400 text-xs hover:text-gray-600">✕</button>
          </div>
        )}
      </div>
      ...
      <div className="bg-white border rounded-sm overflow-hidden" style={{ borderColor: '#e5e5e5' }}>
        <div className="px-4 py-3 border-b flex items-center justify-between"
          ...
          <span className="font-black text-gray-800">
            {proClients.length} client{proClients.length !== 1 ? 's' : ''} Pro
          </span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-max">
                ...
                <tr key={c.id} className="border-t"
                  ...
                  <td className="px-4 py-3">
                    <span className="font-black tracking-widest text-base"
                      style={{ color: COLOR.red }}>{c.accessCode}</span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-xs font-bold px-2 py-0.5 rounded-sm"
                      style={{ background: '#fff8f8', color: COLOR.red }}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1">
                      <button onClick={() => openOverrides(c)}
                        className="p-1.5 rounded-sm hover:bg-red-50" style={{ color: COLOR.red }}
                        title="Gérer les tarifs">
                        <Edit2 size={14} />
                      </button>
                      <button
                        onClick={() => { if (window.confirm(`Supprimer ${c.name} ?`)) deleteProClient(c.id); }}
                        className="p-1.5 rounded-sm hover:bg-red-50 text-red-400" title="Supprimer">
                        <Trash2 size={14} />
                      </button>
                    </div>
